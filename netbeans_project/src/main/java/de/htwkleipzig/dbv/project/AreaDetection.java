...
public class AreaDetection {
	...
	public static void groupArea(ImageProcessor input, int[][] output,
			int height, int width, int x, int y, ImageArea ia,
			HashMap<Integer, Integer> group_map) {
	...
	public static ImageProcessor findAreas(ImageProcessor input,
			int min_pixel_per_group, ArrayList<ImageArea> alia, int color) {
		int height = input.getHeight();
		int width = input.getWidth();
		HashMap<Integer, Integer> group_map = new HashMap<Integer, Integer>();
		...
		for (int x = 0; x < width; x++) {
			for (int y = 0; y < height; y++) {
				if (input.getPixel(x, y) == Glob.PIXELCOLOR_WHITE
						&& output[x][y] == 0) {
